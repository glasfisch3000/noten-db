...
extension UserCommands {
	struct Create: AsyncParsableCommand {
		static let configuration = CommandConfiguration(
			...
			usage: """
				notendb user create [--format <format>] --username <username> --password <password> --level <level>
		...
		@Option(name: .shortAndLong, help: "The new user's permission level.")
		...
		func run() async throws {
			...
			try await withApplicationDBTransaction { db, _ in
				let user = User(username: self.username, password: self.password, level: self.level)
